...
- [x] 1.3 校验 CoordinatorState 恢复字段完整性
  - 确认 `completed_tasks`, `failed_tasks`, `task_vars`, `skill_runs`, `task_dag` 均在 checkpoint 中持久化
  - 用 `aget_state()` 断言验证（`test_coordinator_phase2_checkpoints.py`）
- [x] 1.4 明确 checkpoint 归属 Session 的代码边界
  - Brain 层代码不直接操作 checkpoint 存储结构
- [x] 1.5 清理 `_dag_resume_state` 手动恢复机制
  - `_dag_resume_state` 已全部移除，DAG 状态由 checkpoint 负责持久化

## Phase 2：Session 层独立 + 真相源归位

- [x] 2.1 提取 `SessionRuntime`（含 `emit_event` + `emit_progress` 双接口）
- [x] 2.2 `task_events` 成为 canonical history，`task_runs` 降级为 projection
  - [x] 确认前端 replay contract：`start/step/task/node/checkpoint/brief/plan/question/user_reply/file/error/result/stage_artifacts` 必须 canonical
  - [x] transient 降级白名单：`token/streaming_content/thinking/dag_progress/result_delta/monitoring_live/custom`
  - [x] `review` 事件归属确认：canonical 还是只走 projection/API
- [x] 2.3 清理 TaskRunStore asyncpg.Pool 残留（主路径已用 SessionFactory）
  - [x] `_recover_interrupted_tasks()` 中 `self.pool.fetch()` → SessionFactory
  - [x] `ConversationContextBuilder` 接收 SessionFactory 而非 raw pool
  - [x] 删除 `TaskRunStore.pool` 字段和 `connect()/close()` 中的 pool 管理
- [x] 2.3b Conversation 方法独立安置到 `ConversationService`
- [ ] 2.4 `_runner_tasks` 降级为 process-local cancel handle
  - [x] 新增 `Session.request_cancel()` / `is_cancel_requested()`（已在 SessionRuntime 中定义）
  - [x] `cancel_running_task()` 先写 canonical cancel 事件，再尽力取消本地 runner
  - [ ] 跨实例 cooperative cancel 协议（依赖 Redis 传播）
- [x] 2.5 拆解 `request_payload` JSONB 万能口袋
  - [x] `clarification_history` canonical source → `task_events`
  - [x] SessionRuntime 提供 `get_clarification_history()` 从事件流重建
  - [x] `pending_question` / `nested_interrupt_pending` 拆出为明确 projection
  - [x] `interrupt_state` / `_dag_resume_state` 删除（依赖 Phase 1.5）
- [x] 2.6 TaskService 重构为纯 Harness Runtime
  - [x] 注入 SessionRuntime，`record_event()` 委托给 `session.emit_event()`
  - [x] `_execute_task()` 内所有 `_store.xxx()` 迁移到 `session.xxx()`
  - [x] 删除 TaskService 对 `TaskRunRepository` / `TaskEventRepository` 的直接依赖
- [x] 2.7 明确 Brain 与 Session 边界
...
- [x] 2.8 `_safe_emit` 进度流统一迁移
  ...
  - [x] `direct_answer` 的 ad-hoc token writer → `emit_progress`
  - [x] 验证 `stream_nested_graph()` transport bridge 未被误删
...
- [x] `grep -rn "TaskRunRepository\|TaskEventRepository" agent/` → 零结果
